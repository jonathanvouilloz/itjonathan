...
public class FrmMain extends java.awt.Frame {
    ...
    ModelPays mPays = new ModelPays();
    ModelSport mSport = new ModelSport();
    ModelAthletes mAthlet = new ModelAthletes();
    ...
    private void initObservers () {
        mPays.addObserver(new LstPaysObserver(lstPays));
        mSport.addObserver(new LstSportObserver(lstSports));
        mAthlet.addObserver(new LstAthletesObserver(lstAthletes));
        mAthlet.addObserver(new DetailObserver(tfNo, tfNom, tfPrenom, tfPays, tfSport));
    }
    ...
    private void checkSelPaysSport() {
        if(lstPays.getSelectedIndex() > -1 && lstSports.getSelectedIndex() > -1) {
            mAthlet.loadData((Pays)mPays.get(), (Sport)mSport.get());
        }
    }
    ...
    private void lstAthletesItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_lstAthletesItemStateChanged
        mAthlet.setPos(lstAthletes.getSelectedIndex());
