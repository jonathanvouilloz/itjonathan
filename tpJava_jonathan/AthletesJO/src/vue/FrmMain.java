...
public class FrmMain extends java.awt.Frame {
    ...
    ModelePays mPays = new ModelePays();
    ModeleSport mSport = new ModeleSport();
    ModeleAthlete mAthlete = new ModeleAthlete();
    
    public FrmMain() {
        initComponents();
        initObservers();
        mPays.loadData(); mSport.loadData();
    }
    ...
    private void lstPaysItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_lstPaysItemStateChanged
        mPays.setPos(lstPays.getSelectedIndex());
        selectionPaysSport();
    ...
    private void lstSportsItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_lstSportsItemStateChanged
        mSport.setPos(lstSports.getSelectedIndex());
        selectionPaysSport();    }//GEN-LAST:event_lstSportsItemStateChanged

    private void lstAthletesItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_lstAthletesItemStateChanged
        mAthlete.setPos(lstAthletes.getSelectedIndex());
    ...
    private void initObservers() {
        mPays.addObserver(new PaysObserver(lstPays));
        mSport.addObserver(new SportObserver(lstSports));
        mAthlete.addObserver(new AthleteObserver(lstAthletes));
        mAthlete.addObserver(new DetailsObserver(tfNo, tfNom, tfPrenom, tfPays, tfSport));
    }
    
    private void selectionPaysSport() {
        if(lstPays.getSelectedIndex() > -1 && lstSports.getSelectedIndex() > -1) {
            mAthlete.loadData((Pays)mPays.get(), (Sport)mSport.get());
        }
    }
}